...
// 输入: 总房价、建筑面积
// 二期：单价升值区间： uptoArr = [1.3, 1.5, 1.7, 1.9]
// 计算基数： w = 万
// 服务费比例
const Service_Percent = 0.027;
// 税费
const Tax = 1;
// 所有
const All = 49;
...
const invest = (
  sumPrice,
  area,
  uptoArr = [1.3, 1.4, 1.5, 1.7, 1.9, 2.3, 2.7]
) => {
  console.log("————————————————");
  ...
  // 总支出 = 已支出 + 欠款
  const allOut = alreadyOut + todoLocal;
  !IS_PROD && console.log("总支出", allOut);
  const actualInArr = [];
  // 涨幅 uptoArr
  uptoArr.forEach(upto => {
    ...
    // 实赚
    const actualIn = uptoSumPrice - allOut;
